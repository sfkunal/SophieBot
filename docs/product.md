# Brain — Product Specification

Brain is a shared SMS assistant for a couple. One Twilio number, two allowed phones, one queue for restaurants, one watchlist, and Google Calendar awareness — all with a warm, funny personality that feels like texting a mutual friend, not a bot.

## Problem

Couples accumulate restaurant recommendations and “we should watch that” ideas across texts, notes, and memory. Deciding what to do tonight means digging through old messages, guessing why something was saved, and manually comparing calendars. Brain turns that friction into a single SMS thread plus a lightweight web dashboard.
...
1. **Capture by text** — Save a restaurant or show in one message; Brain extracts metadata and confirms.
...
4. **Personality that sticks** — Replies are short, witty, and emotionally literate; the assistant celebrates wins and gently roasts endless queues.
...
- **Setup** — Phone verification (6-digit SMS code), Google Calendar OAuth per user.
...
All inbound SMS is classified into exactly one intent (see `packages/shared/src/prompts.ts`):
...
Brain texts like a warm, funny mutual friend:
...
- **Phone verification:** 6-digit codes for dashboard setup; only `ALLOWED_PHONES` may register.

## Security & Privacy

- Allowlist enforced on SMS and API.
- `AUTH_SECRET` signs dashboard sessions.
- Google refresh tokens stored encrypted-at-rest in D1 (operator responsibility for secret rotation).
...
- Items captured via SMS per week.
